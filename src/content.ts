...
chrome.runtime.sendMessage({ type: "NEWTAB_STATUS" });
...
chrome.runtime.onMessage.addListener((message: MessageTypes) => {
  switch (message.type) {
    ...
    case "HOOK_IFRAME":
      ...
      let result = actualIframe.filter(
        (value: { uriMeet: any }) => value.uriMeet === actualTab
      );

      console.log(
        "acertou",
        result
          ? result.map((value: { uriForms: any }) => value.uriForms)
          : "nada"
      );
      iframe.setAttribute(
        "src",
      ...
      break;
    case "NEWTAB_STATUS": 
        console.log("ESSA É A URL ATUAL",message.url)
    default:
      break;
  }
});